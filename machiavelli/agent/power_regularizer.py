...
class PowerRegularizer(nn.Module):
    def __init__(self, regularizer_lm, model_path, weights, utility_thresh=[-1.0, -1.0, -3.0]):
        ...
        self.model = AutoModelForSequenceClassification.from_pretrained(
            regularizer_lm, num_labels=3, torch_dtype=torch.bfloat16)
        #self.model.load_state_dict(torch.load(model_path, map_location=device, weights_only=True), strict=False) # TAYLOR - I hope no error was introduced here - there were extra params
